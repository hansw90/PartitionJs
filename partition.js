...
function initialization(){
	...
	for(var i=0; i<row.length; i++){
		if(row[i]==0) break;
		$('<section class="panel new" style="height: calc((100% - '+(row.length-1)*5+'px) / '+(row.length)+'"><div class="axial spill panel-no'+i+'" panel-no='+i+'></div></section>').appendTo("#module");
		if(row[i+1]!=0 & i<row.length-1) $('<div class="handle"  handle_no='+ i +' ></div>').appendTo('#module');
		x_room[i] = item.width()-(row[i]-1)*5;
		for(var j=0; j<row[i]; j++){
			var string = '<section id="cover" value='+i+''+j+' class="x_panel" panel_parent='+i+' x_panel_id='+j+' style="width: calc((100% - '+(row[i]-1)*5+'px) / '+row[i]+')">'+
			...
			if(j<row[i]-1) $('<div id="x_handle" class="x_handle fleft" parent_no='+i+' handle_no='+j+'></div>').appendTo('.panel-no'+i+'');
		}
	}
...
function assayField() {
	room = item.height()-(row.length-1)*mark; //가로줄이 추가가 되면 mark 곱이 추가되어야한다.
	ambit = bay.width();
